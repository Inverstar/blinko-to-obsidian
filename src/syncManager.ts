...
export class SyncManager {
	...
	async startSyncRecycle(): Promise<SyncResult> {
		if (this.isSyncing) {
			this.log('Sync already running, skipping new request.');
			return { newNotes: 0, flashNotes: [] };
		}
		...
		this.isSyncing = true;
		const syncStartTime = Date.now();
		const lastSyncTime = this.settings.lastRecycleSyncTime || 0;
		let page = 1;
		let hasMore = true;
		let newNotes = 0;
		const flashNotes: FlashNoteJournalEntry[] = [];
		...
		try {
			while (hasMore) {
				// eslint-disable-next-line no-await-in-loop
				const notes = await this.client.getRecycleNotes(lastSyncTime, page, this.pageSize);
				if (!notes.length) {
					break;
				}
			...
			this.settings.lastRecycleSyncTime = syncStartTime;
			await this.persistSettings();
			return { newNotes, flashNotes };
		} finally {
			this.isSyncing = false;
